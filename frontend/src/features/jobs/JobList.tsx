...
/**
 * Job list (DESIGN.md §7, T7) — reusable table for a run of jobs, in
 * server order (date desc). Hosted by SpoolDetail now, ProjectDetail in
 * T9 — so it renders whatever the host passes, and owns its own
 * edit/delete dialogs.
 *
...
export interface JobListProps {
  /** The spool context for edit dialogs (the jobs' spool). */
  spool: Spool;
...
/**
 * Render the job list.
 * @param spool The spool the jobs draw from (edit-dialog context).
 * @param jobs The jobs to show, server-ordered.
 * @returns The list (or an empty-state line).
 */
const JobList = ({ spool, jobs }: JobListProps) => {
